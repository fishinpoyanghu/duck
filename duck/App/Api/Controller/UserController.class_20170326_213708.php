...
class UserController extends CommonController
{
    ...
    public function getUser(){
        if(IS_POST){
            $data = I('post.');
            $user = M('member')->field('id,account,status,token,realname as nickname,phone,level,money,currency,action_code,dog_lev,machine,animal_num,enclosure_lev,egg_parent_status,machine_egg,machine_animal,one_clean,crean_time')->where(array('id'=>$this->userId))->find(); 
            if($user['one_clean']){
    ...
    public function activityuser(){
         if(IS_POST){
            $this->checkGet(array('account','nickname','phone'));
           ...
           if (!preg_match('/^1[345789][0-9]{9,9}$/', $post['phone'])) {
            ...
            if($usermsg['animal_num']<$actricePrice){  
            ...
            $result = M('member')->where(array('account'=>$usermsg['account']))->setDec('animal_num',$actricePrice);
...
/*    public function back(){
   ...
   /* public function rsaSign($certPaths,$certPass,$signData)
        ...
        if(IS_POST){
           // $dog_upgradenum=C('dog_upgradenum');
            ...
            if($data['animal_num']<  $dogmsg['updatedog_num']){
                $this->echoJson('升级狗需要'.$dogmsg['updatedog_num'].'只鹅');
            }
    ...
    public function transfer(){
        if(IS_POST){ 
            $this->checkGet(array('type','num','account','super','paypwd')); //1 就是鹅 2是鹅蛋
            $type=I('post.type'); 
            $num=I('post.num')+0; 
            $super=I('post.super');
            $account=I('post.account'); 
            $paypwd=I('post.paypwd'); 
            $friend=M('member')->field('id')->where(array('account'=>$account))->find();
            ...
            if(md5($post['paypwd'])!=$usermsg['highpass']){
                 $this->echoJson('支付密码错入错误！'); 
            }
            //需求更改。订单生产就扣钱。然后交易完成后钱才会到对方那里。
            if($type==1){ //转鹅 15快地全开不用手续费
    ...
    public function gameconfig(){
       if(IS_POST){  
            ...
            $dogmsg=M('dogmsg')->field('lev,updatedog_num as num')->order('lev asc')->select();
